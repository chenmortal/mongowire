...
```bash
cargo test --workspace --all-features   # 全量:239 测试
...
两个目标均以入库的真实流量语料作种子,并各自驱动过真实修复:section 标识符与 `fullCollectionName` 的严格 UTF-8(原 lossy 转换导致长度漂移),以及字符串值内嵌 NUL 的编解码对称性。
